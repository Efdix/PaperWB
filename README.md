...
1. 下载安装向导：完整版 `PaperWB-Setup-x.y.z.exe`（预置模型）或精简版 `PaperWB-Setup-x.y.z-lite.exe`（不预置模型）（发布页）
2. 双击运行。由于个人分发未购买代码签名证书，Windows SmartScreen 可能弹出「Windows 已保护你的电脑」：
   点击 **「更多信息」→「仍要运行」** 即可（本软件不含任何网络服务端，仅调用你配置的 AI 接口）
3. 跟随向导完成安装：
   - **许可协议**：MIT 开源许可
   - **预置模型**：完整版安装即带离线解析模型（约 500 MB）——安装后解析 PDF **完全离线、无需等待下载**；不需要模型请下载精简版，首次解析时自动联网下载（走国内镜像）
   - **安装目录**：默认当前用户目录（`%LOCALAPPDATA%\Programs\PaperWB`，无需管理员权限），也可在向导中改为「所有用户」
   - **完成页**：可勾选「运行安装自检」验证核心组件与离线模型（约 1 分钟，精简版只验证核心组件），以及「立即运行 PaperWB」
...
正式分发物是 **Inno Setup 安装向导**（双版本：完整版 `PaperWB-Setup-<版本>.exe` 预置离线解析模型 + 精简版 `-lite.exe` 不预置模型）。完整版向导流程：许可 → 安装位置 → 数据与缓存目录（可改，写入配置后首次启动不再弹窗）→ 完成页可选安装自检（不再有组件选择页，模型固定安装；不想要模型用精简版）。
...
├── installer/                   # 安装向导构建
│   ├── PaperWB.iss              # Inno Setup 向导脚本（许可/目录/快捷方式/完成页自检；/DLite 出精简版）

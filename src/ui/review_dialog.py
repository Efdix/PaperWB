...
class LogicCheckDialog(QDialog):
    """红线逻辑检查结果对话框 —— 只报致命问题，通过时给出肯定。"""

    def __init__(self, result: dict, parent=None):
        super().__init__(parent)
        self._result = result
        self.setWindowTitle("红线逻辑检查")
        self.resize(560, 480)
    ...
    def _setup_ui(self):
        ...
        issues = self._result.get("issues", []) or []
        ...
        for i, item in enumerate(issues, start=1):
            ...
            ccard.setContentsMargins(12, 8, 12, 8)
            ccard.setSpacing(4)
    ...
    def _copy_result(self) -> None:
        lines = []
        lines.append("红线逻辑检查")
        lines.append(f"通过: {'是' if self._result.get('passed') else '否'}")
        if self._result.get("summary"):
            lines.append(f"总结: {self._result['summary']}")
        for item in self._result.get("issues", []):
            lines.append("-" * 30)
            lines.append(f"[{item.get('type', '问题')}]")
            if item.get("quote"):

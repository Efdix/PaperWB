...
保持引用标记不变：已有引文标记的文本与格式（含上标/方括号形式）原样保留，只随所在句子移动，不重写。
...
- 引用格式必须遵循风格指南中指定的目标期刊格式。该规则只适用于**新增**的引用；原文已有的引文标记一律保留原样——不要改写其格式（上标数字、[1]、(Author, Year) 等都保持原状），不要增删方括号，不要把上标改成正文数字或反之
